...
import velfoxHappy from '/velfox_alive.gif';
import velfoxSleeping from '/velfox_sleep.gif';

const configPath = '/data/adb/.config/Velfox';
const modPath = '/data/adb/modules/velfox';
const binPath = '/data/adb/modules/velfox/system/bin';
const officialWebsite = 'https://t.me/VelocityFox';
const donateUrl = 'https://sociabuzz.com/velocityfox22/tribe?_ref=linkbio';

// WebUI X API
const moduleInterface = window.$velfox;
...
const saveLog = async () => {
  const output = await runCommand(`${binPath}/velfox_utility save_logs`);
  if (output.error) {
...
const getServiceState = async () => {
  ...
  const image = document.getElementById('velfox_logo');
  const pidElement = document.getElementById('daemon_pid');
  const bgDiv = document.getElementById('daemon_bg_image');

  const pid = await runCommand('/system/bin/toybox pidof sec.velfoxruntime || echo null');
  pidElement.textContent = `Daemon PID: ${pid}`;

  if (pid === "null") {
    status.textContent = "Sleep 💤";
    image.src = velfoxSleeping;
    if (bgDiv) bgDiv.style.setProperty('--daemon-bg', `url(${velfoxSleeping})`);
  } else {
    status.textContent = "Alive ✨";
    image.src = velfoxHappy;
    if (bgDiv) bgDiv.style.setProperty('--daemon-bg', `url(${velfoxHappy})`);
  }
};
...
const changeCPUGovernor = async (governor, config) => {
  ...
  if (config === "efficiency_cpu_gov") {
    await runCommand(`[ "$(<${configPath}/current_profile)" -eq 3 ] && velfox_utility change_cpu_gov ${governor}`);
  } else if (config === "custom_default_cpu_gov") {
    await runCommand(`[ "$(<${configPath}/current_profile)" -eq 2 ] && velfox_utility change_cpu_gov ${governor}`);
  }
};
...
const fetchCPUGovernors = async () => {
  ...
  ['default_cpu_gov', 'efficiency_cpu_gov'].forEach(id => {
    const select = document.getElementById(id);
    select.innerHTML = governors.map(gov => `<option value="${gov}">${gov}</option>`).join('');
    select.addEventListener('change', () => changeCPUGovernor(select.value, id.includes('efficiency') ? "efficiency_cpu_gov" : "custom_default_cpu_gov"));
  });

  let default_cpu_gov;
  let efficiency_cpu_gov;
  ...
  if (fileInterface) {
    ...
    efficiency_cpu_gov = fileInterface.read(`${configPath}/efficiency_cpu_gov`).trim();
  } else {
    ...
    efficiency_cpu_gov = await runCommand(`cat ${configPath}/efficiency_cpu_gov`);
  }

  document.getElementById('default_cpu_gov').value = default_cpu_gov;
  document.getElementById('efficiency_cpu_gov').value = efficiency_cpu_gov;
};
...
document.getElementById('velfox_logo').addEventListener('click', () => openWebsite(officialWebsite));
...
const bgDiv = document.getElementById('daemon_bg_image');
if (bgDiv) {
  const img = document.getElementById('velfox_logo');
