...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(base_name, "velfox_log") == 0) {
        if (argc < 3) {
            fprintf(stderr, "Usage: velfox_log <TAG> <LEVEL> <MESSAGE>\n");
    ...
    if (access("/system/bin/dumpsys", F_OK) != 0) {
        ...
        log_velfox(LOG_FATAL, "/system/bin/dumpsys: inaccessible or not found");
    ...
    if (is_file_empty("/system/bin/dumpsys") == 1) {
        ...
        log_velfox(LOG_FATAL, "/system/bin/dumpsys was tampered by kill logger module");
    ...
    if (create_lock_file() != 0) {
        fprintf(stderr, "\033[31mERROR:\033[0m Another instance of Velfox Daemon is already running!\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (access(GAMELIST, F_OK) != 0) {
        ...
        log_velfox(LOG_FATAL, "Critical file not found (%s)", GAMELIST);
        exit(EXIT_FAILURE);
    }

    // Daemonize service
    if (daemon(0, 0)) {
        log_velfox(LOG_FATAL, "Unable to daemonize service");
        exit(EXIT_FAILURE);
    }
    ...
    log_velfox(LOG_INFO, "Daemon started as PID %d", getpid());
    ...
    while (1) {
        ...
        if (access(MODULE_UPDATE, F_OK) == 0) [[clang::unlikely]] {
            log_velfox(LOG_INFO, "Module update detected, exiting.");
        ...
        if (!gamestart) {
            ...
            log_velfox(LOG_INFO, "Game %s exited, resetting profile...", gamestart);
            game_pid = 0;
            free(gamestart);
            gamestart = get_gamestart();
        ...
        if (gamestart && get_screenstate() && mlbb_is_running != MLBB_RUN_BG) {
            ...
            if (game_pid == 0) [[clang::unlikely]] {
                log_velfox(LOG_ERROR, "Unable to fetch PID of %s", gamestart);
                free(gamestart);
                gamestart = NULL;
                continue;
            }
            ...
            cur_mode = EFFICIENCY_MODE;
            need_profile_checkup = false;
            toast("Activating efficiency mode");
            run_profiler(EFFICIENCY_MODE);
