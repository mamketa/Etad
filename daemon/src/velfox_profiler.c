...
void run_profiler(const int profile) {
    ...
    if (systemv("velfox_profiler %d", profile)) {
        log_velfox(LOG_ERROR, "Unable to execute profiler changes to %d", profile);
    }
}
...
bool get_screenstate_normal(void) {
    ...
    fetch_failed++;
    log_velfox(LOG_ERROR, "Unable to fetch current screenstate");

    if (fetch_failed == 6) {
        log_velfox(LOG_FATAL, "get_screenstate is out of order!");
...
bool get_low_power_state_normal(void) {
    ...
    fetch_failed++;
    log_velfox(LOG_ERROR, "Unable to fetch battery saver status");

    if (fetch_failed == 6) {
        log_velfox(LOG_FATAL, "get_low_power_state is out of order!");
