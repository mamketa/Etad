...
char* execute_command(const char* format, ...) {
    ...
    if (pipe(pipefd) == -1) [[clang::unlikely]] {
        log_velfox(LOG_ERROR, "pipe failed in execute_command()");
        return NULL;
    }
    ...
    if (pid == -1) [[clang::unlikely]] {
        close(pipefd[0]);
        close(pipefd[1]);
        log_velfox(LOG_ERROR, "fork failed in execute_command()");
        return NULL;
    }
...
char* execute_direct(const char* path, const char* arg0, ...) {
    ...
    int pipefd[2];
    if (pipe(pipefd) == -1) [[clang::unlikely]] {
        log_velfox(LOG_ERROR, "pipe failed in execute_direct()");
        return NULL;
    }
    ...
    if (pid == -1) [[clang::unlikely]] {
        close(pipefd[0]);
        close(pipefd[1]);
        log_velfox(LOG_ERROR, "fork failed in execute_direct()");
        return NULL;
    }
